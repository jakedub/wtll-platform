...
export default function UmpireSignupPage({ isPublic = false }: { isPublic?: boolean }) {
  ...
  const [publicEnabled, setPublicEnabled] = useState<boolean>(false);
  ...
  useEffect(() => {
    client.get("/umpire/public-config/").then(r => setPublicEnabled(r.data.is_enabled)).catch(() => {});
  }, []);

  const togglePublic = async () => {
    setPublicLoading(true);
    try {
      const r = await client.post("/umpire/public-config/", { is_enabled: !publicEnabled });
      setPublicEnabled(r.data.is_enabled);
    } finally { setPublicLoading(false); }
  };

  const publicURL = `${window.location.origin}/public/umpire-signups`;
      ...
      {!isPublic && (
        <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, mb: 2, p: 1.5, bgcolor: publicEnabled ? "#f1f8f1" : "#fafafa", border: `1px solid ${publicEnabled ? "#2e7d32" : "#e4e4e7"}`, borderRadius: 1.5 }}>
          ...
          <button
            ...
            style={{ padding: "4px 14px", borderRadius: 6, border: "1px solid #ccc", cursor: "pointer", fontSize: "0.78rem", background: publicEnabled ? "#fdecea" : "#e8f5e9", color: publicEnabled ? "#C41230" : "#2e7d32", fontWeight: 600 }}
          >
